...
START_TEST(mult_matrix) {
  ...
  for (int i = 0; i < rows; i++)
    for (int j = 0; j < cols; j++) m.matrix[i][j] = get_rand(-10e5, 10e5);

  for (int i = 0; i < cols; i++)
    for (int j = 0; j < rows; j++) mtx.matrix[i][j] = get_rand(-10e5, 10e5);
  // printf("m:");
  // s21_print_matrix(&m);
  // printf("mtx");
...
START_TEST(mult_matrix3) {
  ...
  matrix_t res = {0};
  ck_assert_int_eq(s21_mult_matrix(&m, &mtx, &res), INCORRECT_CALC);
